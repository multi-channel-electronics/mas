...
/****************************************************************************/
{
	...
	//printf(">> jam_do_wait_microseconds(): I guess we're waiting..\n");
...
)
{
	...
	*/
	...
	if (status)
	{
  	    // If we are running over fibre, and the number of bits to write > MIN_COUNT, then do block reads and writes
	    //if (specified_mce && optimized_mce && count > MIN_COUNT)
  	    if (specified_mce && tdo != NULL)
	    {
			//printf(">> jam_jtag_drscan(): calling scan_mce() because count = %d\n", count);
			scan_mce(count, tdi, tdo);
	    }
	    else
	    {
			...
			for (i = 0; i < count; i++)
			{
				// This call leads to a call to write_mce_single()
...
)
{
	...
	*/
	...
	if (status)
	{
	    ...
	    if (specified_mce && tdo != NULL)
	    {
			//printf(">> jam_jtag_irscan(): calling scan_mce() because count = %d\n", count);
		    scan_mce(count, tdi, tdo);
	    }
	    else
	    {
