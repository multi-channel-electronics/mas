...
int com_port = -1;
BOOL specified_mce = FALSE;
BOOL optimized_mce = TRUE;
int mce_frequency = 10000000;   /* Hz */
...
int jam_jtag_io(int tms, int tdi, int read_tdo)
{
	...
	if (specified_com_port)
	{
		...
		// This call provides spacing between single-bit transactions
		if (tck_delay != 0) delay_loop(tck_delay);
	}
	else if (specified_mce)
	{
		//printf(">> jam_jtag_io(): write_mce_single()!\n");

		// Read_tdo==1 will trigger a flush and read.
		tdo = write_mce_single((tms?1:0), (tdi?1:0), read_tdo);

		// Don't need a delay here, becuase write_mce_single() just adds bits to a stack.
		// The delay for writing to the MCE is in flush_stack()
	}
	else
	{
#if PORT == WINDOWS || PORT == DOS || PORT == UNIX
...
void jam_delay(int32_t microseconds)
{
	...
	//printf(".");
	//printf(">> jam_delay(): I guess we're waiting..\n");

	// This function is called extensively during irscan and drscan to add gaps in between chunks of data written.
	// This must remain here, otherwise .pof programming fails above 5000 Hz.
	delay_loop(microseconds * ((one_ms_delay / 1000L) + ((one_ms_delay % 1000L) ? 1 : 0)));
}
...
)
{
	...
	if (specified_mce)
	{
		fprintf(stderr, "Error:  MCE interface does not support jam_vector_io!\n");
		exit(1);
	}
...
void calibrate_delay(void)
{
...
#if PORT == WINDOWS || PORT == DOS || PORT == UNIX
	// Run the delay measurement DELAY_SAMPLES times
	for (sample = 0; sample < DELAY_SAMPLES; ++sample)
	{
		...
		one_ms_delay += ((DELAY_CHECK_LOOPS * (DWORD)count) / (tick_count1 - tick_count2));
	}

	// Calculate the average number of ticks for 1 ms.
...
int main(int argc, char **argv)
{
...
#if PORT == WINDOWS || PORT == DOS || PORT == UNIX
			// Use mce_frequency (Hz) to set "wb cc tck_half_period ???"
			//   tck_half_period = 50,000,000/2*mce_frequency
			// Use mce_frequency (Hz) for each tms_tdi transaction to set the
			//   delay = num_bits/mce_frequency
			//----------------------------------------
			// jam_process_trst() or jam_execute_statement() calls
			// jam_process_wait()                            calls
			// jam_process_wait_microseconds()               calls
			// jam_do_wait_microseconds()                    calls
			// jam_delay()                                   calls
			// delay_loop()
			//----------------------------------------
			// everything in jamjtag.c calls
			// jam_jtag_io()           calls
			// delay_loop() based on "tck_delay=0," which should be set by jam_set_frequency(), but is never called.  Hmmm.
			//----------------------------------------

			case 'F':
				if (sscanf(&argv[arg][2], "%d", &mce_frequency) != 1)
                                        error = TRUE;

				if (verbose)
				{
				    printf(">> main(): registering a TCK frequency of %d Hz.\n", mce_frequency);
				}
	...
	// Options checking..
#if PORT == WINDOWS || PORT == DOS || PORT == UNIX
	if ((int)specified_lpt_port + (int)specified_com_port + (int)specified_mce > 1)
	{
		fprintf(stderr, "Error:  -s, -p, -u options may not be used together\n\n");
		help = TRUE;
	}
#endif
    ...
    fprintf(stderr, "    -n<card>    : ignored\n");
		...
		fprintf(stderr, "\nTypical MCE commands:\n");
		fprintf(stderr, "    -- For Querying ID Codes\n");
		fprintf(stderr, "    > sudo ./jam -u -v -f8000000 -aread_idcode read_idcode.jam\n");
		fprintf(stderr, "    -- For Configuring FPGA's\n");
		fprintf(stderr, "    > sudo ./jam -u -v -f8000000 -aconfigure sof.jam\n");
		fprintf(stderr, "    -- For Programming EPC16's\n");
		fprintf(stderr, "    > sudo ./jam -u -v -f800000 -aprogram -dDO_VERIFY=0 pof.jam\n");
		fprintf(stderr, "    -- For Programming EPCS64's (Step 1 of 2)\n");
		fprintf(stderr, "    > sudo ./jam -u -v -f8000000 -aconfigure jic.jam\n");
		fprintf(stderr, "    -- For Programming EPCS64's (Step 2 of 2)\n");
		fprintf(stderr, "    > sudo ./jam -u -v -f100000 -aprogram -dDO_VERIFY=0 jic.jam\n");
		fprintf(stderr, "------------------------------------------\n");
		exit_status = 1;
	}
	...
	else
	{
		...
		if (exit_status == 0)
		{
			...
			time(&start_time);
			time_delta = (int) (start_time);
			if (verbose) printf(">> main(): start time = %02u:%02u:%02u\n", time_delta/3600, (time_delta%3600)/60, time_delta%60);
			...
			time_delta = (int) (end_time);
			if (verbose) printf(">> main(): end time = %02u:%02u:%02u\n", time_delta/3600, (time_delta%3600)/60, time_delta%60);

			// Print out elapsed time
			time_delta = (int) (end_time - start_time);
			if (verbose) printf(">> main(): elapsed time = %02u:%02u:%02u\n", time_delta/3600, (time_delta%3600)/60, time_delta%60);
//			if (verbose)
//			{
...
void initialize_jtag_hardware()
{
	...
	{
		// This call needs a frequency parameter
		initialize_mce(mce_frequency);
	}
	else
	{
#if PORT == WINDOWS || PORT == DOS || PORT == UNIX
...
#if PORT == UNIX
		/* Get write permissions on LPT ports from OS */
...
{
	if (specified_mce) {
		fprintf(stderr, "write_byteblaster call, lame.\n");
		exit(1);
	}
...
{
	if (specified_mce) {
		fprintf(stderr, "read_byteblaster call, lame.\n");
		exit(1);
	}
