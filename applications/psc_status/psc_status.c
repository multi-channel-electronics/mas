    ...
    if (!options.read_stdin) {
        ...
        if (mceconfig_open(mce, options.hardware_file, NULL) != 0) {
            if (options.hardware_file)
                fprintf(stderr,
                        "Failed to open hardware configuration file %s\n",
                        options.hardware_file);
            else
                fprintf(stderr,
                        "Failed to open default hardware configuration.\n");
            return 2;
        }
    ...
    for (i = 0; i<16; i++){
        if (i != 0 && i!=1 && i!=5){ //ignore fan speeds and adc_offset readings
            ...
            if (i> 5){
                ...
                printf(" units \t(%5.2f%c)\tNominal %5.2f", tempf, i<11?'V':'A',
                        psc_nominal[i-5]);
            }
            printf("\n");
        }
