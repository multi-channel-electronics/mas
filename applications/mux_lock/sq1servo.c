...
/***********************************************************
 *    sq1servo       : locks sq1 by calculating new sq2_fb value and sweeping
 *                     sq1_bias
...
int main ( int argc, char **argv )
{
    ...
    char init_line1[MAXLINE];    /* record a line of init values and pass it to
                                    genrunfile*/
    char init_line2[MAXLINE];    /* record a line of init values and pass it to
                                    genrunfile*/
    ...
    if (!args_ok && options.argument_opts) {
        ...
        printf ( "   skip_sq1bias (optional) = if specified as 1, no SQ1 bias "
                "is applied\n");
        printf ( "*NOTE*: Make sure sq2fb.init (32 single-entry lines) and "
                "row.init (32 single-entry lines between 0 to 40) are present "
                "in the data directory\n");
        ERRPRINT("no argument specified");
        return ERR_NUM_ARGS;
    }
    if (!args_ok && !options.argument_opts) {
        printf("When ramp parameters are loaded from experiment.cfg, arguments "
                "are:\n"
    ...
    error=genrunfile (full_datafilename, control.filename, 1, control.rc,
            ...
            control.fb, control.dfb, control.nfb, init_line1, init_line2);
    if (error != 0) {
        sprintf(errmsg_temp, "genrunfile %s.run failed with %d",
                control.filename, error);
        ERRPRINT(errmsg_temp);
        return ERR_RUN_FILE;
    }
    ...
    for (j=0; j<control.nbias; j++ ){
        ...
        for (i=-options.preservo; i<control.nfb; i++ ){
            ...
            // Write SQ2 FB
            rerange(temparr, sq2fb, control.column_n, control.quanta,
                    control.column_n);
            ...
            for (snum=0; snum<control.column_n; snum++) {
                ...
                // For printing, below.
                sq1servo.last_frame[snum] = sq1servo.last_frame[idx];
                sq2fb[snum] += control.gain[snum] *
                    ((int)sq1servo.last_frame[snum] - control.target[snum]);
            }
        ...
        i32 *data, int offset, int count)
{
    if (biasing_ac) {
        ...
        write_range_or_exit(mce, m_sq2fb, offset, data, control.column_n,
                "sq2fb");
    }
    return 0;
}
