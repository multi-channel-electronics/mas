...
/***********************************************************
 *    sq1servo_all   : locks sq1 by calculating new sq2_fb for each row while
 *                     sweeping sq1_bias
...
struct {
    ...
    int column_n;   /* Number of columns to servo (and number in readout
                       frame) */
...
int main(int argc, char **argv)
{
    ...
    FILE *bias_fd[MAXROWS];  /* Output files for error and bias entries; one
                                per row */
    ...
    if (!args_ok && options.argument_opts) {
        ...
        printf("   skip_sq1bias (optional) = if specified as 1, no SQ1 bias is "
                "applied\n");
        printf("*NOTE*: Make sure sq2fb.init (32 single-entry lines) is "
    ...
    for (j=0; j<control.nbias; j++ ){
        ...
        for (i=-options.preservo; i<control.nfb; i++ ){
            ...
            for (snum=0; snum<control.column_n; snum++) {
                ...
                write_range_or_exit(mce, m_sq2fb_col+snum, 0, temparr, MAXROWS,
                        "sq2fb_col");
            }
            ...
            for (snum=0; snum<control.column_n; snum++) {
                for (r=0; r<control.rows; r++) {
                    sq2fb[snum][r] += control.gain[snum] *
                        ((int)sq1servo.last_frame[r*control.column_n + snum] -
                         control.target[snum] );
                }
            }
            ...
            if (i >= 0) {
                ...
                for (r=0; r<control.rows; r++) {
                    for (snum=0; snum<control.column_n; snum++)
                        fprintf(bias_fd[r], "%13d ",
                                sq1servo.last_frame[r*control.column_n + snum]);
                    for (snum=0; snum<control.column_n; snum++)
