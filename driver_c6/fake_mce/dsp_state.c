...
int dsp_writemem(dsp_command *cmd, dsp_message *msg)
{
    ...
    switch (cmd->args[0]) {
        case DSP_MEMP:
            if (addr<0 || addr>=PSIZE)
                msg->reply = DSP_ERR;
            else
                dsp_state.p[addr] = val;
            break;

        case DSP_MEMX:
            if (addr<0 || addr>=XSIZE)
                msg->reply = DSP_ERR;
            else
                dsp_state.x[addr] = val;
            break;

        case DSP_MEMY:
            if (addr<0 || addr>=YSIZE)
                msg->reply = DSP_ERR;
            else
                dsp_state.y[addr] = val;
            break;
        default:
            msg->reply = DSP_ERR;
    }
...
void dsp_timer_function(unsigned long arg)
{
    ...
    if (new_frames > state->n_frames)
        new_frames = state->n_frames;
