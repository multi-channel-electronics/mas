...
int dsp_reply_handler(dsp_message *msg, unsigned long data)
{
  ...
  if (dev->state & DDAT_CMD) {
    ...
    PRINT_ERR(dev->card, "unexpected REP received [state=%i, %i %i].\n",
...
void dsp_driver_remove(struct pci_dev *pci)
{
  ...
  if (card >= MAX_CARDS) {
    PRINT_ERR(card, "could not match configured device, ignoring.\n");
    return;
  }
...
void dsp_driver_cleanup(void)
{
  ...
  for(i=0; i < MAX_CARDS; i++) {
    ...
    if ( dev->pci != NULL ) {
      PRINT_ERR(NOCARD, "failed to zero dev->pci for card; %i!\n", i);
    }
  }
#endif
  dsp_ops_cleanup();
