...
int data_qt_configure(int qt_interval, int card)
{
	...
	if (!err)
		err = data_qt_cmd(DSP_QT_DELTA , dframes->frame_size, 0, card);
	
	if (!err)
		err = data_qt_cmd(DSP_QT_NUMBER, dframes->max_index, 0, card);

	if (!err)
		err = data_qt_cmd(DSP_QT_INFORM, qt_interval, 0, card);

	if (!err)
		err = data_qt_cmd(DSP_QT_PERIOD, DSP_INFORM_COUNTS, 0, card);

	if (!err)
		err = data_qt_cmd(DSP_QT_SIZE  , dframes->data_size, 0, card);

	if (!err)
		err = data_qt_cmd(DSP_QT_TAIL  , dframes->tail_index, 0, card);

	if (!err)
		err = data_qt_cmd(DSP_QT_HEAD  , dframes->head_index, 0, card);

	if (!err)
		err = data_qt_cmd(DSP_QT_DROPS , 0, 0, card);
