		  ...
		  loff_t *f_pos)
{
	...
	PRINT_INFO("user demands %li with nonblock=%i\n",
...
int data_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	PRINT_INFO("mapping %#lx bytes to user address %#lx\n",
	       ...
	       unsigned int iocmd, unsigned long arg)
{
	...
	case DATADEV_IOCT_QT_CONFIG:
		PRINT_INFO("configure Quiet Transfer mode "
			   "[inform=%li]\n", arg);
		return data_qt_configure(arg, card);

	case DATADEV_IOCT_QT_ENABLE:
		PRINT_INFO("enable/disable quiet Transfer mode "
			   "[on=%li]\n", arg);
...
int data_ops_init(void)
{
	...
	if (err<=0) {
		PRINT_ERR("could not register_chrdev, "
			  "err=%#x\n", err);
		return err;
	}
	data_major = err;
